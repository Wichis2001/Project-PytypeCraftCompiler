...
from src.Instructions.typed_var import Typed_Var
from src.Instructions.untyped_var import Untyped_Var
...
def p_atrb_expr(t):
    ...
    '''
    if len(t) == 2:
        temp = Identificator(t[1], t.lineno(1), find_column(input, t.slice[1]))
        t[0] = {'_id': t[1], 'expr': temp}
    else:
...
def p_untyped_var_declaration(t):
    '''
    untyped_var_declaration :   LET ID
                            |   LET ID ASSIGN expr
    '''
    if len(t) == 3:
        t[0] = Untyped_Var(t[2], None, t.lineno(2), find_column(input, t.slice[2]))
    else:
        t[0] = Untyped_Var(t[2], t[4], t.lineno(2), find_column(input, t.slice[2]))

def p_typed_var_declaration(t):
    '''
    ...
    '''
    if len(t) == 5:
        t[0] = Typed_Var(t[2], t[4], None, t.lineno(2), find_column(input, t.slice[2]) )
    else:
        t[0] = Typed_Var(t[2], t[4], t[6], t.lineno(2), find_column(input, t.slice[2]) )
...
def p_assign_expr(t):
    ...
    if len(t) == 3:
        tmp = Primitive('number', 1, t.lineno(1), find_column(input, t.slice[1]))
        t[0] = Assign(t[1], t[2], tmp, t.lineno(1), find_column(input, t.slice[1]))
    elif len(t) == 4:
        t[0] = Assign(t[1], t[2], t[3], t.lineno(1), find_column(input, t.slice[1]))
    else:
        t[0] = t[1]
...
def p_for_stmt_loop_var(t):
    ...
    '''
    t[0] = For(t[2], t[4], t[6], t[9], False, t.lineno(1), find_column(input, t.slice[1]))
...
def add_natives(ast):
    ...
    _id = "toLowerCase"
    params = [{'_type': 'any', '_id': 'lower_string'}]
    toLowerCase = ToLowerCase(_id, params, None, stmt_list, -1, -1)
    ast.setFunctions(toLowerCase)
    
    _id = "toUpperCase"
    params = [{'_type': 'any', '_id': 'upper_string'}]
    toUpperCase = ToUpperCase(_id, params, None, stmt_list, -1, -1)
    ...
    type_op = Type_Of(_id, params, 'string', stmt_list, -1, -1)
